...
public class EmailService {
    ...
    private LinkedList<Email> initializeDefaultEmails() {
        var email1 = new Email(this.currentEmailId++, "marcus@example.com", "dasarishiva1@gmail.com", "Hello",
                "Thanks for attending Spring I/O 2022!", LocalDateTime.now());
        var email2 = new Email(this.currentEmailId++, "rob@example.com", "dasarishiva1@gmail.com", "Subject 1",
                "Content 1",
                LocalDateTime.now());
        var email3 = new Email(this.currentEmailId++, "josh@example.com", "dasarishiva.d2@gmail.com",
                "Very high sensitive content", "Content 2", LocalDateTime.now());

        return new LinkedList<>(Arrays.asList(email1, email2, email3));
    }

    public void send(String from, SendEmailRequest request) {
        Email email = new Email(this.currentEmailId++, from, request.to(), request.subject(), request.content(),
                LocalDateTime.now());
